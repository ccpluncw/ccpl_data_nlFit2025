...
for(st in sheets) {
	...
	subs <- unique(data.in$sn)
	targetMultipliers <- unique(data.in$targetMultiplier)
	grades <- unique(data.in$grade)
	...
	for(tm in targetMultipliers) {
		df.tmp.1 <- data.long[data.long$targetMultiplier == tm, ]
		df.tmp.1$target <- df.tmp.1$target * df.tmp.1$targetMultiplier
		lowerBound <- 0
		upperBound <- 1000 * tm
		visibleReferencePoints <- c(lowerBound, upperBound)
		
		for(gd in grades) {		
			df.tmp.2 <- df.tmp.1[df.tmp.1$grade == gd, ]
			...
			for(sn in subs) {
				if(sn == "all") {
					df.tmp <- df.tmp.2 %>% group_by(target) %>% summarize(estimate = mean(estimate, na.rm = T))
				} else {
			  ...
			  sink(paste("..", sinkFileName, sep = "/"), append = TRUE)
			    cat("\n\n ***********************", st, sn, tm, gd, "*********************** \n\n")
				sink(NULL)
				outList <- outputNLfitStats(df.tmp, statList, dataTargetCol = "target", dataEstimateCol = "estimate", pars.n = pars.n, loops = 1000, sinkFilename = paste("..", sinkFileName, sep = "/"), appendSinkFile = TRUE, plotFileName = paste(st, sn, tm, gd,"Plot.pdf", sep="_"))
				...
				df.out.tmp <- data.frame(st, sn, tm, gd, firstEstimate = outList$runStats$parameters$firstEstimate, accuracyPercent = outList$runStats$parameters$accuracyPercent, numberSensitivity = outList$runStats$parameters$numberSensitivity, pIncludeConceptualPoints = outList$runStats$parameters$pIncludeConceptualPoints, rangeLength = outList$runStats$parameters$rangeLength, memoryLength = outList$runStats$parameters$memoryLength, r2 = outList$runStats$fitStats$r2, BIC = outList$runStats$fitStats$BIC, r2.ll, BIC.ll)
